...
3. `main()` - 24 edges
4. `init_run_logger()` - 23 edges
5. `log_event()` - 19 edges
6. `get_dataloaders()` - 17 edges
...
Cohesion: 0.06
Nodes (33): Project Rules (AGENTS.md), Config Centralization Convention, ADR-001: ML Architecture for Cattle & Buffalo Breed Recognition, Bharat Pashudhan App (BPA), CBAM attention module, EfficientNet-Lite2 backbone, Guided image capture overlay, Binary head + species-specific sub-heads (+25 more)
...
### Community 4 - "Documentation Index"
Cohesion: 0.06
Nodes (56): Training Internals & Data Flow — Deep Reference, Evaluation metrics (combined_top1/3/5), Image to tensor pipeline, Label encoding (binary/cattle/buffalo + masks), Loss computation, QAT internals (fuse conv-bn, per-tensor observers), Smoke test data flow, Training loop detail (+48 more)
...
Cohesion: 0.13
Nodes (36): _banner(), build_dataset_inventory(), _check_windows_build_tools(), _detect_nvidia_gpu(), _elapsed(), _file_size_mb(), _install_torch(), _log_event() (+28 more)

### Community 8 - "cattle_buffalo_tester.py"
Cohesion: 0.12
Nodes (8): find_breed_folders(), Walk to find leaf dirs with images (breed folders)., getpass, google_colab, ipython_display, onnxruntime, seaborn, tqdm_notebook
...
### Community 10 - "os"
Cohesion: 0.16
Nodes (17): glob, os, expand_images(), main(), Parity check: PyTorch checkpoint vs fp32 ONNX on a small image list. For each…, soft_top5(), find_latest(), find_latest_checkpoint() (+9 more)
...
### Community 16 - "main"
Cohesion: 0.14
Nodes (19): compute_class_counts(), compute_class_priors(), compute_rare_classes(), _count_per_class(), describe_transform(), _effective_num_weights(), _load_class_maps(), Human-readable list of a Compose's ops (for startup logging). (+11 more)
...
### Community 19 - "cattle_buffalo_trainer.py"
Cohesion: 0.15
Nodes (13): download_kaggle_dataset(), Download and unzip a Kaggle dataset into dest_dir., evaluate_epoch(), _macro_scores(), no_grad, Macro-F1 and macro-recall from a confusion matrix (true x pred). Classes with…, create_portable_export(), Bundle the trained model + labels + metadata into a portable folder. (+5 more)
...
Cohesion: 0.12
Nodes (24): argparse, create_training_zip(), Script to create a lightweight standalone training zip package. Excludes…, Check if relative path matches any exclusion rule., should_exclude(), Path, pathlib, Create archive.zip for Colab upload from local organized dataset. Usage: python… (+16 more)
...
Cohesion: 0.21
Nodes (11): matplotlib, matplotlib_pyplot, numpy, full_evaluation(), main(), no_grad, _save_confusion(), make_run_id() (+3 more)

### Community 27 - "run_logger.py"
Cohesion: 0.08
Nodes (21): atexit, logging, platform, _bootstrap(), _looks_like_project(), Automatic execution-logger bootstrap. Python imports ``sitecustomize`` at…, _dumps(), finish_run() (+13 more)
...
Cohesion: 0.19
Nodes (12): detailed_eval(), load_model(), main(), print_report(), no_grad, Diagnose a trained checkpoint on a split (val or test). Reports, for one or two…, shot_bucket(), get_dataloaders() (+4 more)

### Community 32 - "test_master_cpu.py"
Cohesion: 0.12
Nodes (17): Convert cattle_buffalo_trainer.py to .ipynb notebook format. Parses the `# %%`…, copy, importlib, json, pandas, pil, re, load_index() (+9 more)
...
Cohesion: 0.17
Nodes (19): CBAM attention, Colab README, Colab Training, Model Architecture, EfficientNet-Lite backbone, efficientnet_lite2.pth weights, efficientnet_lite4.pth weights, Kaggle algsoch/breed-cattle-buffalo dataset (+11 more)

### Community 36 - "parity_check.py"
Cohesion: 0.18
Nodes (15): Module Reference — Source Code Cross-Reference, Module dependency graph, _load_model(), accumulate(), evaluate_on_split(), finalize(), main(), make_onnx_runner() (+7 more)
...
Cohesion: 0.18
Nodes (17): Byte caching, Data Pipeline, RandAugment, cutmix(), _make_weighted_sampler(), mixup(), _pairing_perm(), _rand_bbox() (+9 more)

### Community 41 - "train.py"
Cohesion: 0.15
Nodes (23): Automatic Mixed Precision (AMP), Knowledge distillation (teacher-student), Android Deployment, Known Constraints & Gotchas, EMA (Exponential Moving Average), Flutter app, math, ONNX Runtime Mobile INT8 (QDQ) (+15 more)
...
### Community 44 - "prepare_half_splits"
Cohesion: 0.16
Nodes (11): _collect_rows(), prepare_half_splits(), prepare_quarter_splits(), prepare_smoke_splits(), Create a dataset using a fraction of images per breed for faster training. Uses…, Create a dataset using 25% of images per breed for very fast training.…, Create a tiny dataset for smoke tests by sampling a few images per breed. This…, Per-breed train/val/test index lists with hard minimums. A plain round() split… (+3 more)
...
### Community 49 - "create_test_eval_zip.py"
Cohesion: 0.33
Nodes (4): collections, Create a zip file of test-split images for Colab batch evaluation. Reads…, csv, Colab Testing & Evaluation

### Community 51 - "compute_macro_f1"
Cohesion: 0.67
Nodes (3): compute_macro_f1(), Compute macro-averaged F1 across all breeds., Compute macro-averaged F1 across all breeds.

### Community 52 - "plot_confusion_matrix"
Cohesion: 0.67
...
- **Why does `BreedClassifier` connect `BreedClassifier` to `test_master_cpu.py`, `export.py`, `model.py`, `parity_check.py`, `Documentation Index`, `ModelManager`, `train.py`, `os`, `API Reference`, `main`, `._load_model`, `cattle_buffalo_trainer.py`, `evaluate.py`, `test_model.py`, `diagnose_model.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `init_run_logger()` connect `run_logger.py` to `export.py`, `model.py`, `parity_check.py`, `ModelManager`, `local_train.py`, `train.py`, `main`, `evaluate.py`, `test_model.py`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `Common Operations` connect `Documentation Index` to `export.py`, `model.py`, `parity_check.py`, `local_train.py`, `data_pipeline.py`, `train.py`, `API Reference`, `setup_venv.py`, `evaluate.py`, `test_model.py`?**
...
- **Should `CattleBuffaloDataset` be split into smaller, more focused modules?**
