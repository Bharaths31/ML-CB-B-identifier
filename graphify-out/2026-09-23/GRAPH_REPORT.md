...
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 81 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d1aaf5f2`
...
Cohesion: 0.07
Nodes (40): Byte caching, Dataset, Data Pipeline, RandAugment, CattleBuffaloDataset, _collect_rows(), compute_class_priors(), compute_rare_classes() (+32 more)

### Community 1 - "BreedClassifier"
Cohesion: 0.05
Nodes (35): Project Rules (AGENTS.md), Config Centralization Convention, CBAM attention, Architecture & Data Flow, CBAM Attention, Masked Multi-Task Loss, Species Routing Inference, Configuration Reference (+27 more)
...
Cohesion: 0.22
Nodes (7): Model Tester GUI, Presenter configuration, ModelManager, Discovers, loads, and caches models for prediction., Load breed label maps from data/splits/., Find all available checkpoints., Return list of available model names with metadata.
...
Cohesion: 0.05
Nodes (32): compute_macro_f1(), find_breed_folders(), img_to_base64(), per_breed_report(), plot_confusion_matrix(), predict_single(), preprocess_image(), Walk to find leaf dirs with images (breed folders). (+24 more)
...
Cohesion: 0.10
Nodes (29): collections, Create a zip file of test-split images for Colab batch evaluation. Reads…, create_training_zip(), Script to create a lightweight standalone training zip package. Excludes…, Check if relative path matches any exclusion rule., should_exclude(), csv, _absolute() (+21 more)
...
Cohesion: 0.09
Nodes (23): download_kaggle_dataset(), find_breed_dirs(), merge_into_species_dir(), normalize_breed_name(), Download and unzip a Kaggle dataset into dest_dir., Find all leaf directories containing images (breed folders)., Normalize breed folder names: lowercase, underscores, strip whitespace., Auto-detect breed folders inside source_base and copy/merge them into… (+15 more)
...
### Community 23 - "parity_check.py"
Cohesion: 0.18
Nodes (15): _load_model(), Make a checkpoint loadable into a plain float BreedClassifier. Phase-3 QAT…, _sanitize_state_dict(), accumulate(), evaluate_on_split(), finalize(), main(), make_onnx_runner() (+7 more)

### Community 24 - "train.py"
Cohesion: 0.30
Nodes (13): Knowledge distillation (teacher-student), Android Deployment, Known Constraints & Gotchas, EMA (Exponential Moving Average), Flutter app, math, ONNX Runtime Mobile INT8 (QDQ), Export parity gate (+5 more)

### Community 26 - "evaluate.py"
Cohesion: 0.19
Nodes (9): argparse, Common Operations, json, numpy, Create colab_project.zip containing only the files needed for Colab training.…, full_evaluation(), main(), no_grad (+1 more)

### Community 28 - "test_model.py"
Cohesion: 0.15
Nodes (12): base64, datetime, glob, logging, pil_exiftags, struct, extract_image_metadata(), Extract detailed metadata from raw image bytes. (+4 more)
...
Cohesion: 0.36
Nodes (6): Module Reference — Source Code Cross-Reference, Module dependency graph, os, time, torch, torch_nn

### Community 32 - "evaluate_epoch"
Cohesion: 0.22
Nodes (8): evaluate_epoch(), _macro_scores(), no_grad, Macro-F1 and macro-recall from a confusion matrix (true x pred). Classes with…, Train a single phase with AdamW, optional AMP, gradient accumulation, and label…, train_phase(), torch_nn_functional, tqdm

### Community 33 - "run_server"
Cohesion: 0.27
Nodes (9): generate_odt_report(), main(), Run a minimal HTTP server with the GUI and prediction API., Generate an ODT report from prediction results. Args: results: single result…, run_server(), do_GET(), do_POST(), _parse_multipart() (+1 more)
...
- **Why does `BreedClassifier` connect `BreedClassifier` to `data_pipeline.py`, `ModelManager`, `._load_model`, `API Reference`, `ADR-001: ML Architecture for Cattle & Buffalo Breed Recognition`, `cattle_buffalo_trainer.py`, `parity_check.py`, `train.py`, `evaluate.py`, `test_model.py`, `export.py`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `Common Operations` connect `evaluate.py` to `data_pipeline.py`, `BreedClassifier`, `Documentation Index`, `local_train.py`, `API Reference`, `setup_venv.py`, `data_collection.py`, `parity_check.py`, `train.py`, `test_model.py`, `export.py`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `Training Pipeline` connect `Documentation Index` to `train.py`?**
